...
//     const timeOptions_el = document.querySelectorAll(".timeOption").values;
//     timeOptions_el.forEach(function (timeOption) {
...
document.addEventListener("DOMContentLoaded", function(){
    ...
    const timeOptions_el = document.querySelectorAll(".timeOption");

    timeOptions_el.forEach(function (time){
    let timeList = time.innerText;
    let eventDateTime = {[prop]: timeList};
    

    function selectedDateTime(){
        let pairs = Object.values(eventDateTime);
        const result = pairs.filter(eventTime => eventTime === selectedTime);
        // selectedTime.classList.remove(result);
        alert(result);
    }
...
// const eventDate = document.querySelector(".selected-date").innerHTML;
// console.log(eventDate);



// for (let i = 0; i < timeOptions_el.length; i++){
